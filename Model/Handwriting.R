# Head impact and brain acceleration
## Setting
p <- 2
ord <- 2

## Parameter
para <- c(0.00001, rep(0.00001, 46), 0.000005, rep(0.00001, 46))
...
con_state <- function(state, i, time_grid){
  if(i == 1){
    matrix(cbind(state[,1], matrix(sapply(1:46, function(k) (time_grid >= 600 / 46 * (k - 1)) * (time_grid <= 600 / 46 * k)), nrow = nrow(state))), nrow = nrow(state))
  }else{
    matrix(cbind(state[,2], matrix(sapply(1:46, function(k) (time_grid >= 600 / 46 * (k - 1)) * (time_grid <= 600 / 46 * k)), nrow = nrow(state))), nrow = nrow(state))
  }
}
...
dfdp <- function(times, x, p, more){
  ...
  for(k in 1:46){
    resultDtheta[, 3, k + 1] <- (times >= 600 / 46 * (k - 1)) * (times <= 600 / 46 * k)
  }
  
  resultDtheta[, 4, 48] <- x[, 2]
  for(k in 1:46){
    resultDtheta[, 4, k + 48] <- (times >= 600 / 46 * (k - 1)) * (times <= 600 / 46 * k)
  }
  return(resultDtheta)
}
...
fnmodelDtheta <- function(theta, x, tvec){
  ...
  for(k in 1:46){
    resultDtheta[, k + 1, 3] <- (tvec >= 600 / 46 * (k - 1)) * (tvec <= 600 / 46 * k)
  }
  
  resultDtheta[, 48, 4] <- x[, 2]
  for(k in 1:46){
    resultDtheta[, k + 48, 4] <- (tvec >= 600 / 46 * (k - 1)) * (tvec <= 600 / 46 * k)
  }
  return(resultDtheta)
}
