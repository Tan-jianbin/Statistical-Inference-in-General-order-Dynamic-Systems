...
para_est <- function(dat, ord, para_length, int_upp, omega, para, sam_grid, diff, linear) {
  ...
  # Pre-smooth
  time_grid <- seq(0, int_upp, length.out = 5000)
...
prof <- function(int_upp, J, sam_grid, dat, para){
  ...
  DEfd <- smooth.basis(sam_grid, cbind(dat, rep(0, nrow(dat)), rep(0, nrow(dat))), fdPar(bbasis, 2, 1))
  coefs <- DEfd$fd$coefs
  ...
  Result <- Profile.LS(
    ...
    in.meth = 'nlminb',
    out.meth = 'nls',
    control.out = list(trace = T, printEval = T, warnOnly = T, maxiter = 100)
  )
